(() => {
  const PORTAL_BUILD = "11.41-firestore-schema-native";
  ...
  const DEFAULT_YEARS = [
    {
      ...
      sectors: [
        ...
        {name:"Desarrollo social",value:89},
        {name:"Gestión administrativa",value:86},
        {name:"Participación ciudadana",value:82}
      ]
    },
    {
      year: 2026,
      ...
      sectors: [
        {name:"Infraestructura",value:63},
        {name:"Desarrollo social",value:59},
        {name:"Gestión administrativa",value:57},
        {name:"Participación ciudadana",value:51}
      ]
    },
    {
      year: 2027,
  ...
  function normalizeYearsCollection(values) {
    if (!Array.isArray(values)) return [];
    return values.map(year => {
      const normalized = year && typeof year === "object" ? {...year} : {};
      const sectors = Array.isArray(normalized.sectors) ? normalized.sectors : [];
...
window.addEventListener("unhandledrejection",event => {
  ...
  const state = {
    years: normalizeYearsCollection(loadArray(KEYS.years, DEFAULT_YEARS)),
  ...
  const helpers = {
    ...
    save(options = {}) {
      state.years = normalizeYearsCollection(state.years);
...
helpers.showClickEffect = showClickEffect;
  ...
  function loadFirebaseService() {
    ...
    script.src = `firebase-auth-v1141.js?v=${PORTAL_BUILD}`;
